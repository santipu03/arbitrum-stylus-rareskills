...
num=$(cast call --rpc-url $RPC_URL $CONTRACT_ADDRESS "retNumB() (uint256)")
expected_num=$((4))
if [ "$num" -eq "$expected_num" ]; then
    echo -e "${GREEN}retNum test passed.${NC}"
else
    echo -e "${RED}Failed to call ret_num() in contract_b.${NC}"
...
expected_num=$((8))
if [ "$num" -eq "$expected_num" ]; then
    echo -e "${GREEN}foofoo test passed.${NC}"
else
    echo -e "${RED}Failed to call ret_num() in contract_a.${NC}"
